...
class AsyncBot:
    ...
    async def next_step_handler(self, chat_id: int, callback: Callable, *args):
        self._next_step_handlers.update({str(chat_id): {'func': callback, 'args': args}})
            ...
            for update in process_update:
                ...
                try:
                    ...
                    if update.get('message', False):
                        ...
                        for step in self._next_step_handlers:
                            if str(update['message']['chat']['id']) == step:
                                if threaded_run:
                                    executor.submit(asyncio.run, self._next_step_handlers[step]['func'](Message(update['message'], self), *self._next_step_handlers[step]['args']))
                                else:
                                    await self._next_step_handlers[step]['func'](Message(update['message'], self), *self._next_step_handlers[step]['args'])
                                
                                self._next_step_handlers.pop(step)
                                break
